import { Controller, Post, Get, Delete, Req, Res, Headers, HttpCode, HttpStatus, Logger, UseGuards, Query } from '@nestjs/common';
import { Request, Response } from 'express';
import { ConfigService } from '@nestjs/config';
import { AuthGuard } from '@nestjs/passport';
import { ApiTags, ApiOperation, ApiExcludeEndpoint, ApiBearerAuth, ApiOkResponse, ApiQuery } from '@nestjs/swagger';
...
this.logger.log(`rawBody length=${req.rawBody?.length}`);
  ...
  async redirectToInstall(@Req() req: any, @Res() res: Response) {
    const appName = this.config.get<string>('GITHUB_ANALYSIS_NAME') || '16signals-analysis';
    const url = `https://github.com/apps/${appName}/installations/new`;
    this.logger.log(`Redirecting user ${req.user?.id} to GitHub App install: ${url}`);
  ...
  @ApiOperation({
    ...
    description: 'Returns whether the authenticated candidate has installed the GitHub App.',
  })
  @ApiOkResponse({
...
async getPublicStatus(
  ...
  @ApiOperation({
    ...
    description: 'Manually clears the installationId from the authenticated user\'s GithubProfile.',
  })
